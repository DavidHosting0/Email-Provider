...
): Promise<{ raw: Buffer; s3Key: string; bucket: string } | null> {
...
async function resolveRawEmail(
  data: InboundJobData,
): Promise<{ raw: Buffer; s3Ref: string | null }> {
  ...
  try {
    ...
    if (fromS3) {
      ...
      return { raw: fromS3.raw, s3Ref: formatS3Ref(fromS3.bucket, fromS3.s3Key) };
    }
  } catch (err) {
    logger.warn(
